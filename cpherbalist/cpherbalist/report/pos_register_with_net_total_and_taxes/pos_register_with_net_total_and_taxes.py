...
def execute(filters=None):
	...
	if group_by_field:
		non_subtotals = [d for d in grouped_data if not d.get("bold")]

		# total_grand = sum(d.get("grand_total", 0) for d in grouped_data if d)
		# total_net = sum(d.get("net_total", 0) for d in grouped_data if d)
		# total_taxes = sum(d.get("taxes", 0) for d in grouped_data if d)
		# total_paid = sum(d.get("paid_amount", 0) for d in grouped_data if d)
		
		total_grand = sum(d.get("grand_total", 0) for d in non_subtotals)
		total_net = sum(d.get("net_total", 0) for d in non_subtotals)
		total_taxes = sum(d.get("taxes", 0) for d in non_subtotals)
		total_paid = sum(d.get("paid_amount", 0) for d in non_subtotals)

		grouped_data.append({
			group_by_field: _("Total"),
...
def get_pos_entries(filters, group_by_field):
	...
	select_group_by_field = ""
	if group_by_field and group_by_field not in ["mode_of_payment", "owner", "customer"]:
		select_group_by_field = f", p.{group_by_field}"
	...
	return frappe.db.sql(
		f"""
		SELECT
			p.owner, p.customer, p.is_return,
			p.base_grand_total as grand_total,
			p.base_net_total as net_total,
			p.base_total_taxes_and_charges as taxes
			{select_mop_field}
			{select_group_by_field}
...
#     net_total = sum(d.net_total for d in group_invoices)
#     taxes = sum(d.taxes for d in group_invoices)
